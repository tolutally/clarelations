...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    const stats = {
      ...
      totalDealsCreated: recentSyncs?.reduce((sum: number, s: any) => sum + (s.deals_created || 0), 0) || 0,
      totalContactsCreated: recentSyncs?.reduce((sum: number, s: any) => sum + (s.contacts_created || 0), 0) || 0,
      avgProcessingTime: recentSyncs?.length 
        ? recentSyncs.reduce((sum: number, s: any) => sum + (s.duration_ms || 0), 0) / recentSyncs.length
        : 0,
    };
    ...
    res.json({
      ...
      statistics: stats,
      recentSyncs: recentSyncs?.map((sync: any) => ({
